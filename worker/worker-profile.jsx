...
function WProfile({ tick, onSignOut }) {
  ...
  const STAT_CARDS = [
    ...
    { value: `${hours} h`, label: 'Odpracováno', icon: 'clock-circle-bold', color: '#16a34a', bg: 'rgba(22,163,74,0.12)', onClick: () => setHoursOpen(true) },
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ color: T.muted, fontFamily: T.fontUI, fontSize: 13.5, marginTop: 4 }}>
                {lvl.isMax ? <>Maximální level — jsi legenda! <img src="/korunka.png" alt="" style={{ height: '1.1em', verticalAlign: '-0.18em', imageRendering: 'pixelated' }} /></>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 14, marginBottom: 24 }}>
          {STAT_CARDS.map(s => (
            <div key={s.label} onClick={s.onClick} style={{
              ...
              {s.onClick && <Icon name="alt-arrow-right-bold" size={18} color={T.mutedSoft} />}
            </div>
          ))}
        </div>
      ...
      {hoursOpen && <WHoursModal onClose={() => setHoursOpen(false)} />}
    </div>
  );
}

// ── Modál: odpracované hodiny po měsících (z W_HISTORY) ──
function WHoursModal({ onClose }) {
  const now = new Date();
  const [ym, setYm] = useStateW({ y: now.getFullYear(), m: now.getMonth() });

  const MONTHS = ['Leden', 'Únor', 'Březen', 'Duben', 'Květen', 'Červen', 'Červenec', 'Srpen', 'Září', 'Říjen', 'Listopad', 'Prosinec'];
  function prev() { setYm(s => s.m === 0 ? { y: s.y - 1, m: 11 } : { y: s.y, m: s.m - 1 }); }
  function next() { setYm(s => s.m === 11 ? { y: s.y + 1, m: 0 } : { y: s.y, m: s.m + 1 }); }

  const items = (Array.isArray(W_HISTORY) ? W_HISTORY : [])
    .filter(h => {
      if (!h.eventDate) return false;
      const d = new Date(h.eventDate + 'T00:00:00');
      return d.getFullYear() === ym.y && d.getMonth() === ym.m && (h.phase === 'completed' || h.passed);
    })
    .sort((a, b) => (a.eventDate < b.eventDate ? -1 : 1));

  const totalHours  = items.reduce((s, h) => s + ((h.card && h.card.shiftHours) || 0), 0);
  const totalEarned = items.reduce((s, h) => s + ((h.card && h.card.shiftTotal) || 0), 0);
  const roundH = Math.round(totalHours * 10) / 10;

  const navBtn = { width: 36, height: 36, borderRadius: 10, cursor: 'pointer', background: T.surfaceAlt, border: '1px solid ' + T.border, display: 'grid', placeItems: 'center' };
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        boxShadow: '0 24px 60px rgba(20,22,40,0.28)', overflow: 'hidden',
      }}>
        {/* Přepínač měsíců */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '18px 20px', borderBottom: '1px solid ' + T.border }}>
          <button onClick={prev} style={navBtn}><Icon name="alt-arrow-left-bold" size={16} color={T.ink} /></button>
          <div style={{ textAlign: 'center' }}>
            <div style={{ color: T.ink, fontFamily: T.fontHead, fontSize: 17, fontWeight: 800 }}>{MONTHS[ym.m]} {ym.y}</div>
            <div style={{ color: T.muted, fontFamily: T.fontUI, fontSize: 12.5, marginTop: 1 }}>Odpracované hodiny</div>
          </div>
          <button onClick={next} style={navBtn}><Icon name="alt-arrow-right-bold" size={16} color={T.ink} /></button>
        </div>

        {/* Souhrn měsíce */}
        <div style={{ display: 'flex', gap: 12, padding: '16px 20px' }}>
          <div style={{ flex: 1, padding: '14px 16px', borderRadius: 14, background: 'rgba(22,163,74,0.1)' }}>
            <div style={{ color: '#16a34a', fontFamily: T.fontHead, fontWeight: 800, fontSize: 24, letterSpacing: -0.5 }}>{roundH} h</div>
            <div style={{ color: T.muted, fontFamily: T.fontUI, fontSize: 12.5, marginTop: 2 }}>Odpracováno</div>
          </div>
          <div style={{ flex: 1, padding: '14px 16px', borderRadius: 14, background: 'rgba(0,32,246,0.08)' }}>
            <div style={{ color: T.primary, fontFamily: T.fontHead, fontWeight: 800, fontSize: 24, letterSpacing: -0.5 }}>{totalEarned.toLocaleString('cs-CZ').replace(/,/g, ' ')}</div>
            <div style={{ color: T.muted, fontFamily: T.fontUI, fontSize: 12.5, marginTop: 2 }}>Vyděláno Kč</div>
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px 20px' }}>
          {items.length === 0 ? (
            <div style={{ padding: '28px 16px', textAlign: 'center', color: T.mutedSoft, fontFamily: T.fontUI, fontSize: 13.5 }}>
              V tomto měsíci nemáš žádnou dokončenou brigádu.
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {items.map(h => {
                const d = new Date(h.eventDate + 'T00:00:00');
                const hrs = (h.card && h.card.shiftHours) || 0;
                const earned = (h.card && h.card.shiftTotal) || 0;
                return (
                  <div key={h.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 14, background: '#fff', border: '1px solid ' + T.border }}>
                    <div style={{ width: 42, height: 42, borderRadius: 12, background: T.surfaceAlt, display: 'grid', placeItems: 'center', flexShrink: 0 }}>
                      <div style={{ textAlign: 'center', lineHeight: 1 }}>
                        <div style={{ color: T.ink, fontFamily: T.fontHead, fontWeight: 800, fontSize: 15 }}>{d.getDate()}.</div>
                        <div style={{ color: T.mutedSoft, fontFamily: T.fontUI, fontSize: 9.5, fontWeight: 700 }}>{d.getMonth() + 1}.</div>
                      </div>
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ color: T.ink, fontFamily: T.fontHead, fontSize: 14.5, fontWeight: 800, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{h.jobTitle}</div>
                      <div style={{ color: T.muted, fontFamily: T.fontUI, fontSize: 12.5, marginTop: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{[h.company, h.timeText].filter(Boolean).join(' · ')}</div>
                    </div>
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      <div style={{ color: T.ink, fontFamily: T.fontHead, fontSize: 14, fontWeight: 800 }}>{hrs ? hrs + ' h' : '—'}</div>
                      {earned > 0 && <div style={{ color: T.muted, fontFamily: T.fontUI, fontSize: 12 }}>{earned.toLocaleString('cs-CZ').replace(/,/g, ' ')} Kč</div>}
                    </div>
                  </div>
                );
              })}
        ...
        <button onClick={onClose} style={{ margin: '0 20px 20px', padding: '13px', borderRadius: 14, background: T.surfaceAlt, border: '1px solid ' + T.border, color: T.muted, fontFamily: T.fontHead, fontSize: 14.5, fontWeight: 800, cursor: 'pointer' }}>Zavřít</button>
      </div>
    </div>
  );
}
