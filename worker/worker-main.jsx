...
// Sleduje, jestli je okno široké (desktop) → sidebar vs spodní lišta
function useIsWideW(bp) {
  const [wide, setWide] = useStateW(() => typeof window !== 'undefined' && window.matchMedia('(min-width:' + bp + 'px)').matches);
  useEffectW(() => {
...
function WorkerApp() {
  const [tab,    setTab]    = useStateW('home');
  const isWide = useIsWideW(900);
  ...
  const NAV = [
    { id: 'home',     label: 'Domů',     icon: 'home-smile-bold' },
    { id: 'swipe',    label: 'Práce',    img: '/employer/jobs-icon.png' },
    { id: 'history',  label: 'Brigády',  icon: 'checklist-minimalistic-bold', badge: reviewsToDo },
    { id: 'calendar', label: 'Kalendář', img: '/employer/calendar-icon.png' },
    { id: 'messages', label: 'Zprávy',   img: '/employer/messages-icon.png', badge: unreadMessages },
    { id: 'profile',  label: 'Profil',   img: '/employer/user.png' },
  ];
  // černé PNG ikony → firemní modrá (aktivní) / šedá (neaktivní)
  const NAV_BLUE = 'brightness(0) saturate(100%) invert(13%) sepia(100%) saturate(4000%) hue-rotate(228deg) brightness(103%)';
      ...
      {loaded && isWide && (
        <aside style={{
          ...
          <div style={{ padding: '4px 12px 24px' }}>
            <span style={{ fontFamily: T.fontHead, fontWeight: 900, fontSize: 23, color: T.primary, letterSpacing: -0.5 }}>Makej!</span>
          </div>
              ...
              return (
                <button key={n.id} onClick={() => setTab(n.id)} style={{
                  ...
                  <div style={{ position: 'relative', display: 'grid', placeItems: 'center' }}>
                    {n.img
                      ? <img src={n.img} alt="" style={{ width: 21, height: 21, objectFit: 'contain', flexShrink: 0, filter: active ? NAV_BLUE : 'opacity(0.4)' }} />
                      : <Icon name={n.icon} size={21} color={active ? T.primary : T.mutedSoft} />}
                    {n.badge > 0 && (
                      <span style={{
                        position: 'absolute', top: -6, right: -8, minWidth: 16, height: 16, padding: '0 4px',
                        borderRadius: 999, background: T.primary, color: '#fff', fontSize: 9, fontWeight: 800,
                        fontFamily: T.fontUI, display: 'grid', placeItems: 'center', border: '2px solid ' + T.card,
                      }}>{n.badge}</span>
                    )}
                  </div>
                  <span>{n.label}</span>
            ...
            return (
              <button
                ...
                <div style={{ position: 'relative' }}>
                  {n.img
                    ? <img src={n.img} alt="" style={{ width: 22, height: 22, objectFit: 'contain', filter: active ? NAV_BLUE : 'opacity(0.4)' }} />
                    : <Icon name={n.icon} size={22} color={active ? T.ink : T.mutedSoft} />}
                  {n.badge > 0 && (
                    <span style={{
